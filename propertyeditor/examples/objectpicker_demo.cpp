...
int main(int argc, char *argv[])
{
    ...
    ObjectHighlighter persistentHighlighter;
    ...
    QObject::connect(clearHighlightButton, &QPushButton::clicked, &window,
                     [&persistentHighlighter, pickedObjectLabel]() {
                         persistentHighlighter.clear();
                         pickedObjectLabel->setText(QStringLiteral("Current Pick: <none>"));
                     });
    QObject::connect(&objectPicker, &ObjectPicker::objectPicked, &window,
                     [&persistentHighlighter, pickedObjectLabel](QObject *targetObject) {
                         persistentHighlighter.setTargetObject(targetObject);
                         pickedObjectLabel->setText(QStringLiteral("Current Pick: %1")
                                                        .arg(describeObject(targetObject)));
                     });
    QObject::connect(&objectPicker, &ObjectPicker::activeChanged, &window,
                     [&persistentHighlighter](bool active) {
                         if (active) {
                             persistentHighlighter.clear();
                         }
                     });
