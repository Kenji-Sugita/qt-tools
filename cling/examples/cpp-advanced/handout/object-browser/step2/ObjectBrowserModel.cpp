...
QVariant ObjectBrowserModel::data(const QModelIndex& index, int role) const
{
    ...
    QObject* object = 0; // TODO: Now how do you get to the object for the index?
...
QModelIndex ObjectBrowserModel::parent(const QModelIndex& index) const
{
    ...
    return createIndex(row, 0, parent);  // When index points to object
    // return createIndex(row, 0, grandParent);  // When index points to parent
}
