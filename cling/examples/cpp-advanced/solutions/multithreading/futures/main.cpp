...
static QFuture<QPainterPath> visible(const Container<QPolygonF>& c, const QRectF& window)
{
    return QtConcurrent::filteredReduced(c, IntersectTest(window), build_path);
}
...
int main(int argc, char** argv)
{
...

    {
        ...
        timer.start();
        {
            QFutureSynchronizer<double> sync;
            for (int i = 0; i < ITERATIONS; ++i) {
...

    {
        ...
        timer.start();
        {
            QFutureSynchronizer<QRegion> sync;
            for (int i = 0; i < ITERATIONS; ++i) {
...

    {
        ...
        timer.start();
        {
            QFutureSynchronizer<QPainterPath> sync;
            for (int i = 0; i < ITERATIONS; ++i) {
                sync.addFuture(visible(polygons, referenceF));
            }
        }
