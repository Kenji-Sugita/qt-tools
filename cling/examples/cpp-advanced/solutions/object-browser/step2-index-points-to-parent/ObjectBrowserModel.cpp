...
QModelIndex ObjectBrowserModel::index(int row, int column, const QModelIndex& parent) const
{
    ...
    return createIndex(row, column, qobjectFromModelIndex(parent));
}
...
/*
 * The internal pointer of the model index points to the parent object that the model index is an index for.
 */
