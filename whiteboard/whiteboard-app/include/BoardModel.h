...
public:
    ...
    void addImage(const QRectF &rect, const QByteArray &imageData, const QString &mimeType);
    ...
    bool reconnectSelectedLineEndpoint(int vertexIndex, const QPointF &point, qreal hitRadius);
    ...
    bool applyElementsToPage(const QVector<DrawingElement> &elements,
                             PageElementApplyMode mode,
                             QHash<QString, QString> *assignedIds = nullptr);
    bool updateElementsById(const QVector<DrawingElement> &elements);
    bool deleteElementsById(const QStringList &ids, QStringList *detachedConnectorIds = nullptr);
    bool applyElementChanges(const QVector<DrawingElement> &additions,
    ...
    bool importPages(const QVector<Page> &pages, QString *errorMessage = nullptr);
...
private:
    void pushUndoState();
    void clearRedoState();
    DrawingElement makeElement(ElementType type);
