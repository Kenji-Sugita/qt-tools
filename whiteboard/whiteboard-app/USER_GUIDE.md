...
直線は図形へ接続でき、接続先の図形を動かしても線端点が追従します。
...
| 分類 | 機能 |
| --- | --- |
| 描画 | ペン、直線、コネクター、矩形、角丸矩形、楕円、円、折れ線、ベジェ曲線、円弧、テキスト |
...
| 保存 | 終了時の自動保存と復元、画像取り込み、現在ページの PNG/SVG 画像保存 |
...
| 項目 | 内容 |
| --- | --- |
| アプリ | WhiteboardApp 1.2.0 |
| UI フレームワーク | Qt 6 Widgets |
| ビルドシステム | CMake 3.16 以上 |
| C++ | C++17 |
| Qt コンポーネント | Core, Gui, HttpServer, Network, Widgets, Test |
| MCP 実装 | WhiteboardApp 専用の最小 JSON-RPC / Streamable HTTP 実装 |
...
バージョンは `whiteboard-app/VERSION` を正本とします。CMake の project version、アプリケーションバージョン、MCP の `serverInfo.version` はこの値に合わせます。リリース内容は `whiteboard-app/CHANGELOG.md` に記録します。

リポジトリ直下の `whiteboard-app.zip` はローカル確認用の生成物で、Git 管理しません。正式な source release ZIP は `whiteboard-app/release/whiteboard-app-<version>-source.zip` に作成し、ZIP 内のトップ階層は `whiteboard-app-<version>/` とします。`DECISIONS.md`、`NEXT.md`、`backlog.md`、`docs/`、`release/` は source release ZIP に含めません。
...
| `WhiteboardCoreSelectionTest` | 選択、移動、コピー、グループ、コネクター追従 |
| `WhiteboardCoreDocumentTest` | 保存対象、ページ、設定、描画要素 ID、コネクター ID 張り替え |
| `WhiteboardImageExporterTest` | PNG/SVG の範囲、背景、選択要素、SVG ID、画像埋め込み、拡張子、保存処理 |
| `WhiteboardImageImporterTest` | PNG/JPEG/BMP/SVG 取り込み、SVG の PNG 化、画像要素の保存読み込み |
...
接続確認では、単純な `GET http://127.0.0.1:8765/mcp` は使用しません。WhiteboardApp の
Streamable HTTP MCP Server は GET の SSE ストリームを提供しないため、この確認では
`405 Method Not Allowed` が返るのが正常です。疎通確認は JSON-RPC の `initialize` を
POST して行います。
...
正常時は `HTTP/1.1 200 OK` と `Mcp-Session-Id` ヘッダーが返ります。

従来の標準入出力接続が必要な MCP クライアントでは、実行ファイルへ `--mcp` を付けて起動できます。
この互換モードでは、そのクライアントが WhiteboardApp のプロセスを起動します。
MCP Server は WhiteboardApp 内に実装されているため、外部の qtmcpserver ソースツリーは不要です。
...
| `whiteboard/elements/list` | 現在ページまたは指定ページの要素と永続 ID を取得 | 不要 |
| `whiteboard/elements/update` | 現在ページの要素を永続 ID で一括更新 | 必要 |
| `whiteboard/elements/delete` | 現在ページの要素を永続 ID で一括削除 | 必要 |
| `whiteboard/elements/apply` | 現在ページの要素を一度に追加・更新・削除 | 必要 |
...
Codex へ WhiteboardApp 上の描画を依頼する場合は、HTML や `visualize` ではなく
WhiteboardApp の MCP tool を使うことを明示してください。例:

```text
WhiteboardApp の MCP ツールを使って、現在ページに色 red、線幅 5px、半径 50px の正円を追加してください。
HTML や visualize は使わないでください。
```

`whiteboard/elements/update` と `whiteboard/elements/delete` は、すべての ID と変更内容を検証してから
現在ページへ一括反映します。存在しない ID、重複 ID、不正な属性が一つでも含まれる場合は何も変更しません。
一括更新または一括削除も 1 回の Undo になります。要素の `id` と `type` は更新できません。
画像要素は `rect`、`rotationDegrees`、`groupId` の更新に対応します。
要素一覧と状態取得は文書全体の `revision` を返します。更新・削除・統合変更では、その値を
`expectedRevision` に指定してください。読み取り後に GUI や別の MCP 操作で文書が変わっていれば、
古い状態による変更は拒否されます。`whiteboard/elements/apply` は追加・更新・削除を合計 500 件まで
一度に反映し、追加要素の `clientId` と新しい永続 ID の対応を返します。
...
- 線の開始点または終了点が矩形、角丸矩形、楕円、円、テキスト、画像の上にある場合、線端点はその図形へ接続されます。
...
| 概念 | 内容 |
| --- | --- |
| `BoardDocument` | ページ列、現在ページ、設定、次に使う描画要素 ID 採番値を持つ保存単位 |
| `Page` | 1 枚のキャンバス。描画要素、ズーム、スクロール位置、ロック状態を持つ |
| `DrawingElement` | 線、図形、テキスト、画像などの描画要素。保存 JSON 上では永続 ID を持つ |
| `BoardSettings` | 現在のツール、色、線幅、フォント、ウィンドウサイズなどの設定 |
| 選択状態 | UI 操作用の一時状態。保存対象ではない |

描画要素の永続 ID は `element-1`, `element-2` のように自動で割り当てます。古い保存ファイルに ID がない場合は、読み込み時に ID を補完します。この補完では図形の見た目、座標、重なり順は変更しません。コピー、貼り付け、MCP の図反映で新しく作られる要素には、コピー元や入力元とは異なる新しい ID を付けます。接続済み直線は、接続先図形の永続 ID を保存します。
...
| ファイルメニュー | 画像ファイルの取り込み、現在ページを PNG または SVG 画像として保存 |
...
| コネクター | Line ツールで図形上から、または図形上へドラッグ | 線端点が図形境界へ接続され、図形移動やサイズ変更に追従 |
...
| Paste | Ctrl+V / Command+V | コピーしたオブジェクトを少しずらして貼り付け。アプリ内コピーがない場合は OS クリップボード画像を貼り付け |
...
接続済み直線と接続先図形をまとめてコピー、貼り付けした場合、貼り付け後の直線は貼り付け後の図形へ接続します。
接続先を含まずに直線だけ貼り付けた場合、その接続は解除されます。
...
Line ツールで直線を作成するとき、開始点または終了点を矩形、角丸矩形、楕円、円、テキスト、画像の上に置くと、その端点が図形へ接続されます。
接続済み端点は図形の境界上に表示され、図形を移動、サイズ変更、回転、スケーリングしても追従します。

接続済み直線または接続解除済み直線の端点を図形上へドラッグして離すと、その端点を再接続できます。
図形外で離した場合は自由端点のままです。
直線をリサイズした場合は、操作した端点の接続を解除します。
接続先図形を削除した場合、直線は残り、削除された接続先への接続だけが解除されます。
...
File メニューの `Import Image...` から、PNG、JPEG、BMP、SVG の画像ファイルを現在ページへ取り込めます。
スクリーンショットなど OS クリップボードに入っている画像は、Paste で直接貼り付けられます。
対応画像ファイルはキャンバスへドラッグ＆ドロップして取り込めます。

ファイル選択と貼り付けでは表示中範囲の中央に配置され、ドラッグ＆ドロップではドロップ位置を基準に配置されます。取り込んだ画像は通常のオブジェクトとして選択、移動、サイズ変更、回転、コピー、ペースト、グループ化、削除ができます。大きい画像はキャンバス内に収まる初期サイズへ縮小されます。小さい画像は拡大されません。

SVG ファイルはベクター図形へ分解せず、読み込み時に PNG 画像へ変換して画像オブジェクトとして扱います。画像データは保存ファイル内に埋め込まれるため、`whiteboard.json` を他のホストへ移しても取り込んだ画像は残ります。

画像オブジェクトは線幅、線種、塗り潰し、矢印、フォント、スタイルリセットの対象外です。ページロック中は画像取り込み、画像貼り付け、画像ファイルのドラッグ＆ドロップは実行できません。
...
出力画像は対象オブジェクトを囲む範囲へ自動的に切り抜かれ、線幅、矢印、回転後の形状も欠けないように調整されます。現在のズーム倍率とスクロール位置は出力へ影響しません。PNG は表示用の画像、SVG は通常の描画要素をベクターのまま保持し、取り込んだ画像だけを `<image>` として埋め込む画像として保存されます。
...
「ファイル」→「描画データをエキスポート…」から「全ページ」または「現在のページ」を選び、
`.whiteboard` ファイルへ保存します。選択中の要素だけでなく、指定したページ全体が対象です。
画像もファイル内に埋め込まれるため、画像ファイルを別に渡す必要はありません。

取り込む側では「ファイル」→「描画データをインポート…」でファイルを選びます。
ファイル内のページが、現在のページの直後へ順番に追加されます。既存の描画は残ります。
旧版の自動保存 `whiteboard.json` も選択できます。

- 図形・テキスト・画像・回転・重なり順・グループ・コネクター接続・ページロックを保持します。
- ページごとのズームとスクロール位置を保持します。選択ツールやウィンドウサイズは変更しません。
- 取り込み後も元のページを表示します。追加ページ数を通知し、1回の Undo で取り込み全体を取り消せます。
- 取り込み後に20ページを超える場合は、ページを切り詰めずエラーにします。
- 現在ページがロック中、または既に20ページある場合はインポートできません。
- エキスポートはロック中や空ページでも利用できます。編集中のテキストは書き出す前に確定します。
- 未対応の形式や破損ファイルは取り込まず、エラーを表示します。
- 保存先に同じフォントがない場合、テキストは代替フォントで表示されることがあります。

`.whiteboard` は再編集用です。閲覧用の画像が必要な場合は、従来の「画像を保存…」で PNG/SVG を保存してください。
