...
void CanvasWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (tool == Tool::Select) {
        if (m_dragMode == DragMode::Move)
            ...
            m_model->reconnectSelectedLineEndpoint(m_polylineVertexIndex, canvasPoint, handleSize() * 2.0);
        }
...
{
    if (importDroppedImage(event->mimeData(), event->position())) {
        event->acceptProposedAction();
        notifyChanged();
    }
}
...
bool CanvasWidget::canAcceptImageDrop(const QMimeData *mimeData) const
{
    if (!m_model || m_model->currentPageLocked() || !mimeData)
        return false;
...
bool CanvasWidget::importDroppedImage(const QMimeData *mimeData, const QPointF &position)
{
    ...
    const QPointF canvasPoint = toCanvasPoint(position);
    if (mimeData->hasImage()) {
        const QImage image = imageFromMimeData(mimeData);
        if (!ImageImporter::importFromImage(image, QSize(BoardModel::CanvasWidth, BoardModel::CanvasHeight), canvasPoint, &element, &errorMessage))
            return false;
    ...
    for (const QUrl &url : mimeData->urls()) {
        ...
        if (!isSupportedImageFile(filePath))
            continue;
        if (!ImageImporter::importFromFile(filePath, QSize(BoardModel::CanvasWidth, BoardModel::CanvasHeight), canvasPoint, &element, &errorMessage))
            continue;
        m_model->addImage(element.rect(), element.imageData(), element.imageMimeType());
        return true;
    }
