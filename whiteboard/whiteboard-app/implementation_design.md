...
- CMake ターゲットは次の構成を維持する。
  ...
  - `WhiteboardImageImporterTest`: PNG/JPEG/BMP/SVG 取り込み、QImage 取り込み、SVG の PNG 化、画像要素の保存読み込みの Qt Test。
...
- `WhiteboardCore` は `Qt6::Core`、`Qt6::Gui`、`Qt6::Svg` に依存し、`Qt6::Widgets` に依存しない。
- `WhiteboardPlatform` は `Qt6::Widgets` に依存してよい。
- `WhiteboardApp` と `WhiteboardCanvas*Test` は `WhiteboardCore` と `Qt6::Widgets` に依存する。
- `WhiteboardApp`、`WhiteboardMcpTest`、`WhiteboardMcpHttpTest` は外部 qtmcpserver に依存しない。
  MCP の JSON-RPC 処理と stdio 行区切り処理は `WhiteboardMcpController` が担当する。
...
- バージョンは `VERSION` ファイルを正本とし、CMake の project version、`WHITEBOARD_APP_VERSION`
  compile definition、アプリケーションバージョン、MCP `serverInfo.version` に反映する。
- リリース変更履歴は `CHANGELOG.md` に記録する。
- リポジトリ直下の `whiteboard-app.zip` は Git 管理外のローカル確認用生成物とし、正式な
  source release ZIP は `release/whiteboard-app-<version>-source.zip` に置く。`DECISIONS.md`、
  `NEXT.md`、`backlog.md`、`docs/`、`release/` は source release ZIP に含めない。
...
- 新規描画要素、貼り付け要素、MCP 図反映で追加する要素には `BoardDocument` から採番した永続 ID を付ける。
- 直線端点の接続は `DrawingElement` の接続先 ID として保持し、`BoardModel::refreshConnectors()` で現在ページ内の接続先要素から端点座標を再計算する。
- 接続済み直線は既存の `ElementType::Line` として扱い、専用の描画要素型や専用ツールは追加しない。
- `addLine()` は開始点または終了点が矩形、角丸矩形、楕円、円、テキスト、画像上にある場合、該当端点へ接続先 ID を設定し、端点を対象境界へ正規化する。
- 接続先要素の移動、サイズ変更、回転、スケール、テキストサイズ変更、角丸変更、削除後は `refreshConnectors()` を呼び、端点の追従または失効した接続 ID の解除を行う。
- 接続済み直線または接続解除済み直線の端点を手動で移動して図形上で離した場合は、その図形へ接続 ID を設定する。図形外で離した場合は、操作対象端点の接続 ID を解除したままにする。直線をリサイズする場合は、操作対象端点の接続 ID を解除する。
- 貼り付けと MCP 図反映では、入力要素の ID を一時 ID として扱い、採番後の永続 ID へ接続先 ID を張り替える。接続先が同時入力または Append 時の既存ページに存在しない場合、その接続は解除する。
...
- 次に作成する描画要素 ID の採番値を保持し、`element-1`, `element-2` 形式で割り当てる。
- 読み込み時や外部から文書を受け取った時は、既存の一意な ID を保持し、欠落または重複した ID だけを補完する。
- ページ数は最大 20 件に制限する。
...
- 描画要素 1 個の永続 ID、型、点列、矩形、テキスト、画像データ、画像 MIME type、色、塗り潰し、線幅、角丸矩形の丸み、
  線種、フォント、閉じた折れ線/ベジェ曲線、矢印、直線端点の接続先 ID、円弧角を保持する。
...
- 画像要素は矩形ジオメトリを持ち、PNG の Base64 データを `imageData`、MIME type を
  `imageMimeType` として保存する。

### `DrawingRenderer`

- `DrawingElement` の画面表示と PNG 出力で共有する `QPainter` 描画を担当する。
- 自由曲線、直線、折れ線、ベジェ曲線、矩形、角丸矩形、楕円、円、円弧、テキスト、
  画像、塗り潰し、点線、回転、端点矢印を同じ描画規則で処理する。
...
- SVG は文字列として通常の描画要素のベクター要素を生成し、画像要素だけを Base64 data URI の
  `<image>` として埋め込む。
  描画要素に永続 ID がある場合は SVG の外側 `g` の `id` に使い、ID がない直接生成要素だけ
  要素種別とページ内の 1 始まり index によるフォールバック ID を使う。
...
- `WhiteboardCore` に属し、Qt Widgets に依存せず、PNG、JPEG、BMP、SVG のファイル取り込みを担当する。
- PNG、JPEG、BMP は `QImageReader` で読み込み、SVG は `QSvgRenderer` で PNG 画像へ描画する。
- OS クリップボード画像やドロップされた画像 MIME は `QImage` 入力として受け取り、同じ PNG 画像要素へ変換する。
- 内部保存形式は PNG Base64 に統一し、画像要素の `imageMimeType` は `image/png` とする。
- 初期配置はファイル選択と貼り付けでは表示中範囲の中央、ドラッグ＆ドロップではドロップ位置を基準とし、キャンバス内に収まるよう大きい画像だけを縮小する。小さい画像は拡大しない。
...
- コピー/ペースト用のアプリ内クリップボードは `QVector<DrawingElement>` として `MainWindow` が保持する。
  `Paste` はアプリ内コピーを優先し、アプリ内コピーがない場合は OS クリップボード画像を画像要素として貼り付ける。
...
- MCP の図反映（`whiteboard/diagram/apply`）は全モードで自動承認する。
  要素更新・削除・統合変更、ページ追加・削除・ロック、画像ファイル保存、Undo/Redo は
  `QMessageBox` でユーザー承認を得てから実行する。変更後は通常の保存処理を呼ぶ。
  HTTP Server の開始失敗は警告し、GUI 本体の起動は継続する。
- メニューバーは File、Edit、Help の順とし、File に `Import Image...` と `Save Image...` を置く。
  `Import Image...` はページロック中に無効化し、取り込み成功時は通常の保存処理を呼ぶ。
  キャンバスは対応画像ファイルのドラッグ＆ドロップを受け付け、ページロック中や非画像ドロップは拒否する。
...
- WhiteboardApp 専用の最小 MCP JSON-RPC dispatcher として、標準入出力または
  1 HTTP セッション分のメッセージ処理を提供する。
- 外部 qtmcpserver や QObject/QEvent の汎用操作 API を使わず、アプリ固有 tool だけを公開する。
- 公開ツールは状態・要素一覧、永続 ID による要素の一括更新・削除、ページ一覧、ページ移動・追加・
  削除・ロック、図の一括反映、現在ページ PNG、PNG/SVG の取得・保存、Undo/Redo の
  統合要素変更を含む Whiteboard 固有 16 件に限定する。
- MCP の `initialize`、`tools/list`、`tools/call` と `notifications/initialized` だけを処理し、
  その他の method は JSON-RPC error として返す。
- MCP JSON と `DrawingElement` の変換、入力上限、キャンバス境界の検証を担当する。
- MCP の `whiteboard/diagram/apply` は任意の `id`、直線専用の `startConnectionElementId`、
  `endConnectionElementId` を受け付ける。入力 `id` は一時参照用であり、実際に保存する ID は
  `BoardDocument` から採番したものに置き換える。
- 図反映は全入力を変換・検証した後に `BoardModel::applyElementsToPage()` を 1 回だけ呼ぶ。
- 要素更新と削除は現在ページを対象とし、全 ID と全変更内容を検証してから `BoardModel` へ
  一括反映する。`id` と `type` は変更せず、成功した 1 呼び出しを 1 Undo とする。
- `BoardDocument::revision` は文書変更ごと、および Undo/Redo ごとに単調増加させ、保存 JSON に保持する。
  要素更新・削除・統合変更は `expectedRevision` の一致を反映前に検証する。
- 統合変更は追加の `clientId`、永続 ID による更新・削除を一括処理し、追加要素の採番対応を返す。
...
- SVG は画像要素以外の各 `DrawingElement` を対応する SVG 図形または path、text として出力する。
  画像要素は Base64 data URI の `<image>` として出力する。要素ごとの外側 `g` には永続 ID を付ける。
...
- 描画要素のキーは次を維持する。
  ...
  - `startArrowHead`
  - `endArrowHead`
  - `startConnectionElementId`
  - `endConnectionElementId`
  - `arcStartAngle`
  - `arcSpanAngle`
- 色は `QColor::HexArgb` 形式で保存する。
- `id` は保存ファイル内の描画要素を識別する永続 ID とし、通常は `element-N` 形式にする。
- `nextElementId` は次回採番用の正整数とし、既存 ID と衝突しない値へ補正する。
- 選択状態、Undo/Redo スタック、一時入力状態は保存しない。

## 描画データ交換契約

- `BoardDocumentExchange` は独立したヘッダ／ソースで形式識別、JSON検証、ページ復元と
  `QSaveFile` による書き出しを担当する。WhiteboardCore に含め、GUIに依存させない。
- 交換ファイルは `format = WhiteboardApp.DrawingData`、`formatVersion = 1`、`pages` を持つ。
  `BoardDocument::toJson()` の操作設定や採番状態をそのままコピーしない。
- `BoardModel::importPages()` はロックとページ上限を検証し、コピー上で新規 ID 採番と
  ページ内コネクター参照変換を準備した後、1回のUndo記録で追加する。現在ページと操作設定は維持する。
- `MainWindow` は範囲選択、ファイル選択、未確定テキスト確定、結果通知、UI更新と通常保存を担当する。
- 外部入力には起動時復元の `loadFromFile()` を使わない。起動時復元の暗黙補正・20ページ切り詰め・
  Undo履歴消去を外部インポートへ持ち込まない。
- 専用テスト `WhiteboardDocumentExchangeTest` で全型・画像の往復、旧形式、二重取り込み、
  ID・接続維持、Undo/Redo、revision、上限、ロック、破損データと保存失敗を確認する。
...
- 保存 JSON に `id` がない既存描画要素は読み込み時に永続 ID を補完する。重複 ID は一意になるよう補正し、図形の見た目、座標、重なり順は変更しない。
...
- MCP テストでは公開ツールが 16 件であること、要素一覧・更新・削除・統合変更、競合拒否、ページ一覧・移動・管理、画像取得・保存、
...
- 画像保存テストでは選択要素だけの出力、PNG/SVG の同一範囲、透明/白背景、回転と線幅の
  欠け防止、SVG のベクター要素、画像要素の `<image>` 埋め込みと永続 ID、拡張子規則、
  設定ダイアログの既定値を確認する。
- 保存読み込みテストでは描画要素の永続 ID、旧保存ファイルの ID 補完、貼り付けと MCP 図反映での新 ID 採番、コネクター接続先 ID の張り替えを確認する。
- 画像取り込みテストでは PNG/JPEG/BMP/SVG の取り込み、QImage 入力、SVG の PNG 化、画像要素の保存読み込みを確認する。
  キャンバステストでは画像ファイルのドラッグ＆ドロップとページロック時の拒否を確認する。
