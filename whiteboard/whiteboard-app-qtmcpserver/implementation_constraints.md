...
- MCP 対応は `/usr/local/qt/qtmcpserver` のソースを CMake の `add_subdirectory()` で組み込み、
  `qtmcpserver` ターゲットへリンクする。別の配置は `QTMCPSERVER_SOURCE_DIR` で指定できるようにする。
- WhiteboardApp は `QMcpServer::BuiltinToolPackMode::CustomOnly` を使用する。
