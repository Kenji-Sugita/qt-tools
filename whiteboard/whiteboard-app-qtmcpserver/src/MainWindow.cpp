                                   ...
                                   QString *denyReason)
{
    ...
    if (toolName == QStringLiteral("whiteboard/diagram/apply")) {
        ...
        const int elementCount = arguments.value(QStringLiteral("elements")).toArray().size();
        requestText = tr("Allow AI to apply %1 objects using mode '%2'?")
                          .arg(elementCount)
                          .arg(mode);
...
void MainWindow::updateUiState()
{
    ...
    const bool fillEnabled = canEditPage && (!hasSelection || supportsFillControl(selectedType, selectedClosed));
    const bool arrowEnabled = canEditPage && (!hasSelection || supportsArrowControls(selectedType, selectedClosed));
    ...
    if (m_strokeSpinBox) {
        const int strokeWidth = (hasSelection && !textSelected)
        ...
        m_strokeSpinBox->setEnabled(canEditPage && !textSelected);
    ...
    if (m_strokeStyleComboBox) {
        const StrokeStyle strokeStyle = (hasSelection && !textSelected)
        ...
        m_strokeStyleComboBox->setEnabled(canEditPage && !textSelected);
    }
    if (m_startArrowComboBox && m_endArrowComboBox) {
    ...
    if (m_colorAction)
        m_colorAction->setEnabled(canEditPage);
    if (m_fillAction)
        m_fillAction->setEnabled(fillEnabled);
    if (m_fontAction)
        m_fontAction->setEnabled(canEditPage && (!hasSelection || textSelected));
    if (m_resetStyleAction)
        m_resetStyleAction->setEnabled(canEditPage);
    if (m_scaleAction)
    ...
    if (m_pasteAction)
        m_pasteAction->setEnabled(canEditPage && !m_clipboardElements.isEmpty());
    if (m_groupAction)
    ...
    if (m_floatingPasteButton)
        m_floatingPasteButton->setEnabled(canEditPage && !m_clipboardElements.isEmpty());
    if (m_pageLabel) {
